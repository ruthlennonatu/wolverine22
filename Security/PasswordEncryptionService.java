     ...
     public static byte[] getEncryptedPassword(String password, byte[] salt)
         throws NoSuchAlgorithmException, InvalidKeySpecException
         {
          ...
          Scanner keyIn = new Scanner(System.in);
            String password, passwordAttempt = new String();

            password = "Password1234";
            byte[8] salt = generateSalt();
            byte[] encryptedPass = getEncryptedPassword(password, salt);
                                        
            passwordAttempt = keyIn.nextLine();
            boolean password = authenticate(passwordAttempt, encrytedPass, salt);                   
           //Print out the password, the attempted password, the salt and whether they match or not.
            if (password == true)
            {
            System.out.print("The password match:");
            }
            else
            {
